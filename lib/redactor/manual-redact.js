...
(function() {
  ...
  function applyLiveDomMasks(domBoxes) {
    ...
    domBoxes.forEach((box, idx) => {
      ...
      mask.className = 'ps-mr-dom-mask ps-injected';
      mask.setAttribute('data-ps-ignore', 'true');
      mask.style.left = `${Math.round(box.x + scrollX)}px`;
      mask.style.top = `${Math.round(box.y + scrollY)}px`;
      mask.style.width = `${Math.round(box.width)}px`;
      mask.style.height = `${Math.round(box.height)}px`;
      
      const token = box.tokens && box.tokens[0] ? box.tokens[0] : `[MANUAL_REDACT_${idx + 1}]`;
      mask.innerHTML = `
        <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px;">${token}</span>
        <button type="button" class="ps-mr-mask-del" title="Remove redaction" style="background:none;border:none;color:#94a3b8;cursor:pointer;padding:0 2px;font-size:10px;line-height:1;margin-left:auto;display:inline-flex;align-items:center;">✕</button>
      `;

      mask.querySelector('.ps-mr-mask-del').addEventListener('click', (e) => {
        e.stopPropagation();
        mask.remove();
        activeDomMasks = activeDomMasks.filter(m => m !== mask);
      });
