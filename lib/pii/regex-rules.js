...
(function() {
  ...
  const PIIRules = [
    ...
    // NOTE: Verhoeff checksum is NOT used as a hard gate.
    // Privacy-first design: ANY 12-digit number matching Aadhaar format MUST be redacted.
    // Verhoeff failures could mean the page has a typo, test data, or a novel valid number.
    // Under-redaction is far worse than over-redaction for a privacy tool.
    {
      id: 'AADHAAR',
      name: 'Indian Aadhaar UID',
      tokenPrefix: 'AADHAAR',
      priority: 95,
      // Matches: XXXX XXXX XXXX, XXXX-XXXX-XXXX, XXXXXXXXXXXX (12 digits starting 2-9)
      pattern: /\b[2-9]\d{3}[\s\-]?\d{4}[\s\-]?\d{4}\b/g,
      // No validate() → every structural match is redacted (privacy-first)
    },
